...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!contract_id) return new Response(JSON.stringify({ error: "Missing contract_id" }), { status: 400, headers: CORS });
    ...
    if (cErr || !contract) {
      return new Response(JSON.stringify({ error: "Contract not found or not accepted" }), { status: 404, headers: CORS });
    }
    if (contract.business_id !== user.id) {
      return new Response(JSON.stringify({ error: "Only the business can initiate payment" }), { status: 403, headers: CORS });
    }
    if (!contract.amount_cents || contract.amount_cents <= 0) {
      return new Response(JSON.stringify({ error: "Contract has no payment amount" }), { status: 400, headers: CORS });
    }
